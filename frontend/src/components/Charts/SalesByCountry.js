...
class Example2 extends PureComponent {
    ...
    findCountries = () => {
        let countriesList = {}
        this.props.data.filter(m => m.sold).map(m => countriesList[m.country] ? null : countriesList[m.country] = 0)
        this.props.data.filter(m => m.sold).map(m => countriesList[m.country]++)
    ...
    render() {
        ...
        return (
            ...
            <div className='SalesByParameters'>
                <h3 className='salesByCountry'>Sales By Country</h3>
                <BarChart
                    width={660}
                    height={160}
                    data={country}
